...
impl<'a> VecSerializer<'a> {
    ...
    fn encode_unsigned(&mut self, n: u64, major_type: u8) -> Vec<u8> {
        match n {
            n if n < MAX_U8  => self.encode_u8(n as u8, major_type),
            n if n < MAX_U16 => self.encode_u16(n as u16, major_type),
            n if n < MAX_U32 => self.encode_u32(n as u32, major_type),
            n if n < MAX_U64 => self.encode_u64(n as u64, major_type),
            _ => unreachable!()
        }
    }

    fn encode_u8(&self, value: u8, major_type: u8) -> Vec<u8> {
        match value as u64 {
            n @ 0 ... 23 => vec![bytes::concat(major_type, n as u8)],
            n @ 24 ... 255 => vec![bytes::concat(major_type, 24), n as u8],
            _ => unreachable!()
        }
    }

    fn encode_u16(&self, value: u16, major_type: u8) -> Vec<u8> {
    ...
    fn encode_u32(&self, value: u32, major_type: u8) -> Vec<u8> {
    ...
    fn encode_u64(&self, value: u64, major_type: u8) -> Vec<u8> {
        let mut header = vec![bytes::concat(major_type, 27)];
        let mut value: Vec<u8> = unsafe {
            transmute::<u64, [u8; 8]>((value).to_be()).to_vec()
        };
        header.append(&mut value);
        header
    }
...
impl<'a> Serializer for VecSerializer<'a> {
    ...
    fn serialize_bytes(&mut self, bytes: Vec<u8>) {
        self.serialize_unsigned(bytes.len() as u64, 0b010);
        self.writer.write_bytes(bytes);
    }
...
#[test]
fn serialize_u64() {
    let value = Value::Int(0x100000000000000 as u64);
    assert_eq!(vec![27, 1, 0, 0, 0, 0, 0, 0, 0], to_bytes(value));
}
